...
from app.models.schemas import TagType, Permission, TagConfig
...
def validate_tag_config(tag_config: TagConfig) -> TagConfig:
    """태그 설정 유효성 검증"""
    # 대소문자 구분 없이 TagType 검증
    tag_type = tag_config.tag_type.upper()
    if tag_type not in [dt.value.upper() for dt in TagType]:
        valid_types = ", ".join([dt.value for dt in TagType])
    ...
    if tag_type != TagType.ANALOG.value.upper():
    ...
    # 정확한 enum 값으로 변환
    tag_config.tag_type = next(dt for dt in TagType if dt.value.upper() == tag_type)
