...
# Modbus 서버 자동 시작 스크립트
# 작성자: 자동 생성
# 목적: 시스템 재부팅 시 Modbus 서버를 자동으로 시작
...
echo "$(date '+%Y-%m-%d %H:%M:%S') - Modbus 서버 시작 중..."
...
# 기존 실행 중인 서버 확인 및 종료
PID=$(ps aux | grep "python main.py\|uvicorn main:app" | grep -v grep | awk '{print $2}')
if [ ! -z "$PID" ]; then
    echo "$(date '+%Y-%m-%d %H:%M:%S') - 기존 서버 프로세스($PID) 종료 중..."
    kill -9 $PID
    sleep 2
fi

# 서버 시작 (로그는 애플리케이션 자체에서 관리)
echo "$(date '+%Y-%m-%d %H:%M:%S') - Python 서버 실행 시작"
nohup python main.py > /dev/null 2>&1 &

# 새 프로세스 ID 기록
NEW_PID=$!
echo "$(date '+%Y-%m-%d %H:%M:%S') - 서버가 PID $NEW_PID로 시작되었습니다"
